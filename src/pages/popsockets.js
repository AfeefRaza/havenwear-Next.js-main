...
const popsockets = ({product}) => {
  ...
  <div className="container px-5 py-24 mx-auto">
    <div className="flex flex-wrap -m-4">
      {Object.keys(product).length === 0 && <p>All of the popsockets are currently out of stock, Stay tuned</p>
      }
      { Object.keys(product).map((item)=>{
        ...
        <div className="mt-4 text-center">
          <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">popsockets</h3>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category: "popsockets"})
  let popsockets = {}
    
    for(let item of products){
      if (item.title in popsockets) {
        if (!popsockets[item.title].color.includes(item.color) && item.availableQty > 0) {
          popsockets[item.title].color.push(item.color)
        }
        if (!popsockets[item.title].size.includes(item.size) && item.availableQty > 0) {
          popsockets[item.title].size.push(item.size)
        }
        
      } else {
        popsockets[item.title] = JSON.parse(JSON.stringify(item))
        if (item.availableQty > 0) {
          popsockets[item.title].color = [item.color]
          popsockets[item.title].size = [item.size]
        }
      }
    }
  return {
    props: {product: JSON.parse(JSON.stringify(popsockets)) }, 
  }
}
