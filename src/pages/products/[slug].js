...
const Post = ({Buynow,addtoCart, product, variants, error}) => {
 ...
 const refreshVariant = (nsize,ncolor) =>{
  ...
  return <>
  <ToastContainer />
  <section className="text-gray-600 lg:h-4/5 body-font overflow-hidden">
  <div className="container lg:h-[85vh] px-5 py-24 mx-auto">
    <div className="lg:w-5/5 lg:h-[85vh]  mx-auto flex flex-wrap">
      <img alt="ecommerce" className="lg:w-1/2 lg:h-5/6 w-full  object-cover object-center  border-solid border-4 border-text" src={product.img}/>
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        {/* <h2 className="text-2xl title-font text-gray-500 tracking-widest">HavenWear</h2> */}
        <h1 className="text-gray-900 text-5xl title-font font-medium mb-1 border-text border-b-4 pb-4">{product.title} ({product.size}/{product.color})</h1>
        <div className="flex">
        {product.availableQty > 0 && <span className="title-font font-medium text-6xl mb-5 mt-5 text-text">Rs.{product.price}</span>}
          {product.availableQty <=0 && <span className="title-font font-medium text-6xl text-red-700">OUT OF STOCK!</span>}
        </div>
        
        <p className="leading-relaxed font-market">{product.desc}</p>
        ...
        <div className="flex">
          ...
          <button disabled={product.availableQty <=0} onClick={() => {Buynow(product.slug,1,product.title,product.price,size,color,product.img)}} className="flex w-48 h-16 text-center md:ml-0 md:mt-2 text-background disabled:bg-primary-50 bg-primary border-0 flex items-center justify-center p-0 text-2xl hover:text-text     hover:bg-secondary border-solid border-2  border-text">Buy Now!</button>
          <button disabled={product.availableQty <=0} onClick={() => {addtoCart(product.slug,1,product.title,product.price,size,color,product.img)}} className="flex ml-5 md:ml-5 md:mt-2 bg-secondary   text-text disabled:bg-secondary-50  border-0 flex items-center justify-center w-48 h-16 border-solid border-2 text-2xl   border-text  hover:text-background   hover:bg-text ">Add to Cart</button>
        ...
        {/* <div className="pin mt-5 flex">
          <input type="text" onChange={onChangepin} placeholder="Enter your pincode" className='border-2 mr-2 border-gray-300 rounded-md' />
